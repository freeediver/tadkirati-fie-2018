...
class EventCheckInController extends MyBaseController
{
    ...
    public function postCheckInAttendee(Request $request)
    {
        ...
        if ((($checking == 'in') && ($attendee->has_arrived == 1)) || (($checking == 'out') && ($attendee->has_arrived == 0))) {
            return response()->json([
                'status'  => 'error',
                'message' => 'Ticket déjà validé ' . (($checking == 'in') ? '(à ' . $attendee->arrival_time->format('d/m H:i') . ')' : 'Out') . '!',
                'checked' => $checking,
                'id'      => $attendee->id,
            ]);
        }
    ...
    public function postCheckInAttendeeQr($event_id, Request $request)
    {
        ...
        if (is_null($attendee)) {
            return response()->json([
                'status'  => 'error',
                'message' => "Ticket invalide! Veuillez réessayer."
            ]);
        }
        ...
        if ($attendee->has_arrived) {
            return response()->json([
                'status'  => 'error',
                'message' => 'Le participant s\'est déjà enregistré à ' . $attendee->arrival_time->addHours(1)->format('d/m H:i') . $appendedText
            ]);
        }
        ...
        return response()->json([
            'status'  => 'success',
            'message' => 'Ticket valide !<br>Nom: ' . $attendee->first_name . ' ' . $attendee->last_name . '<br>Référence: ' . $attendee->reference . '<br>Ticket: ' . $attendee->ticket . '.' . $appendedText
        ]);
    }
    ...
    public function confirmOrderTicketsQr($event_id, $order_id)
    {
        ...
        return response()->json([
            'message' => $updateRowsCount . ' Participant(s) Enregistré(s).'
        ]);
    }
